...
CentroidSize <- list()
  ...
  CentroidSize[[ 3 ]] <- summary( procD.pgls(f1=Centroid.size.L ~ Family * Body.Mass.log , phy = phy, data = gdf, iter = 1000, SS.type = "II") )
  ...
  CentroidSize[[ 7 ]] <- summary( procD.pgls(f1=Centroid.size.L ~ Body.Mass.log * Endosurface.log , phy = phy, data = gdf, iter = 1000, SS.type = "I") )
  CentroidSize[[ 8 ]] <- summary( procD.pgls(f1=Centroid.size.L ~ Body.Mass.log * PostRostral.Cranium.length , phy = phy, data = gdf, iter = 1000, SS.type = "II") )
  CentroidSize[[ 9 ]] <- summary( procD.pgls(f1=Centroid.size.L ~ Family, phy = phy, data = gdf, iter = 1000, SS.type = "I") )
  CentroidSize[[ 10 ]] <- summary( procD.pgls(f1=Centroid.size.L ~ Diet, phy = phy, data = Anseriformgdf, iter = 1000, SS.type = "I") )
  
InnerEarShape <- list()
   
  InnerEarShape[[ 1 ]] <- summary(procD.pgls(f1=shape ~ Centroid.size.L* Body.Mass.log, phy = phy , data = gdf , SS.type = "I", iter = 1000)) #Change Centroid.size.L to Centroid.size.dis to check discrete
  ...
  InnerEarShape[[ 4 ]] <- summary(procD.pgls(f1=shape ~ Flight + Family , phy = phy, data = gdf, SS.type = "I", iter = 1000)) #This is tested as SS.type I and II  
  InnerEarShape[[ 5 ]] <- summary(procD.pgls(f1=shape ~ Diet, phy = phy, data = NonDromgdf, SS.type = "I", iter = 1000)) #Change the data to Anseriformgdf to test only within Anseriformes
  ...
  InnerEarShape[[ 9 ]] <- summary(procD.pgls(f1=shape ~ CC.Width.log * CC.Height.log , phy = phy, data = gdf, SS.type = "II", iter = 1000))
